"""R5: exits. Fixed TP (tp_r=4) vs ATR TRAILING, per coin, on the survivors.

Does letting winners run with a trailing stop beat a wide fixed TP? FIXED entry
config per strategy (the validated one); only the exit changes. OOS = 2025->.
Metric in R.
"""
...
# (strategy -> entry_params, max_hold, coins) for the survivors/candidates
ENTRY = {
...
def main():
    ...
    L = ["# R5 - Fixed TP vs trailing, per coin (survivors)",
         f"_{datetime.now(timezone.utc):%Y-%m-%d %H:%M} UTC | fixed validated entry | only the exit changes | "
         f"OOS=2025-> | exp_R_\n",
         "| Strategy | Coin | fixed_tp4 full/OOS | trail1.5 full/OOS | trail2.0 full/OOS | trail3.0 full/OOS |",
         "|---|---|---|---|---|---|"]
