"""Does the market regime gate improve the edge? Comparative backtest (06-29).

Runs the honest engine for each portfolio combo TWICE, without the regime filter
(OFF) and with it (ON, benchmark = BTC vs 4h EMA50), and compares exp_R / sum_R /
winrate per independent OOS window:

  W1 holdout  : [2025-01-01, 2026-01-01)   <- outside the param selection
  W2 2026-YTD : [2026-01-01, now)

Gold (cluster 'gold') is EXEMPT (uncorrelated with crypto) and reported as such,
without the filter. Read-only: does not touch the DB or the deployment.

Usage:  python -m research.regime_backtest
"""
...
W2 = (1767225600000, 10_000_000_000_000)  # 2026-YTD (open end)
...
def main():
    ...
    print(f"Regime: {config.market_benchmark} close>EMA{config.market_regime_ema} "
          f"@ {config.market_regime_tf_h}h | {reg_ts.size} bars | "
          f"bullish {100*reg_bull.mean():.0f}% of the time\n")

    hdr = f"{'COMBO':<26}{'WIN':<5}| {'OFF  n   expR   sumR':<22}| {'ON   n   expR   sumR':<22}| d_exp"
    ...
    for sym, a in all_assignments():
        ...
        for wname, (lo, hi) in (("W1", W1), ("W2", W2)):
            ...
            tag = " (anti-beta = exempt)" if exempt else ""
    ...
    print("=" * len(hdr))
    print("AGGREGATE (sum of R over every combo):")
    for w in ("W1", "W2"):
        ...
        print(f"  {w}: OFF sumR {o:>+7.2f} (n={n_o}, exp {eo:+.3f})  ->  "
              f"ON sumR {c:>+7.2f} (n={n_c}, exp {ec:+.3f})  | d_exp {ec-eo:+.3f}")
